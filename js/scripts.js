...
$(document).ready(function () {
  ...
  const toppingArr = ["cheese", "pepperoni", "olives", "fried chicken", "banana", "chicken livers", "hot pocket", "mushrooms", "candy canes", "artichoke", "anchovy", "pineapple"];
  let newPizza = new Pizza();
  let clickCounter = 0;
  $("#btn-toppings, #btn-size").hide();
  ...
  $("#btn-small, #btn-medium, #btn-large, #btn-XL").click(function (event) {
    ...
    $("#btn-right, #btn-add, #btn-checkout, #btn-toppings, #btn-size").show(1000);
  ...
  $("#btn-add").click(function (event) {
    //Ensure index is not out of bounds when array clickCounter resets
    if (toppingArr[clickCounter - 1] != undefined) {
      $("#output-text-area").text(`Added ${toppingArr[clickCounter - 1]} to the pizza!`)
        (newPizza.toppings.push(" " + toppingArr[clickCounter - 1]) + " ")
    }
    else if (toppingArr[clickCounter] != toppingArr[clickCounter[-1]]) {
      $("#output-text-area").text(`Added ${toppingArr[clickCounter + 11]} to the pizza!`)
        (newPizza.toppings.push(" " + toppingArr[clickCounter + 11]) + " ")
    }
    else {
      $("#output-text-area").text(`Added ${toppingArr[clickCounter]} to the pizza!`)
